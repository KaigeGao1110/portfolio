    ...
    <div style={{ position:'fixed', inset:0, zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}
      ...
      <div style={{ position:'relative', background:'#fff', borderRadius:20, padding:'clamp(1.5rem,4vw,3rem)', maxWidth:640, width:'100%', maxHeight:'85vh', overflowY:'auto', boxShadow:'0 20px 60px rgba(0,0,0,0.15)' }}
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:20 }}>
          {project.tags.map(t => <span key={t} className="kg-tag" style={{fontSize:11}}>{t}</span>)}
        </div>
        {project.stats && (
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))', gap:'clamp(1.25rem,3vw,2.5rem)' }}>
          {projects.map(p => (
            <div key={p.slug} onClick={() => setSelected(p)}
              style={{ position:'relative', background:'#fff', border:'1px solid rgba(0,0,0,0.08)', borderRadius:16, padding:'clamp(1.25rem,3vw,2.5rem)', boxShadow:'0 1px 3px rgba(0,0,0,0.06),0 4px 16px rgba(0,0,0,0.04)', cursor:'pointer', transition:'all 0.3s', overflow:'hidden' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor='rgba(59,130,246,0.4)'; e.currentTarget.style.transform='scale(1.02)'; e.currentTarget.style.boxShadow='0 4px 24px rgba(0,0,0,0.08)'; e.currentTarget.querySelector('.p-title').style.color='#3b82f6'; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor='rgba(0,0,0,0.08)'; e.currentTarget.style.transform='scale(1)'; e.currentTarget.style.boxShadow='0 1px 3px rgba(0,0,0,0.06),0 4px 16px rgba(0,0,0,0.04)'; e.currentTarget.querySelector('.p-title').style.color='#1a1a1a'; }}>
              {/* Glow */}
              ...
              <div style={{ position:'relative', zIndex:1 }}>
                <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:16 }}>
                  <div>
                    <h3 className="p-title" style={{ fontSize:20, fontWeight:700, color:'#1a1a1a', marginBottom:3, transition:'color 0.3s' }}>{p.title}</h3>
                  ...
                  <div style={{ display:'flex', gap:6 }}>
                    <a href={p.url} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()}
                       style={{ width:32, height:32, border:'1px solid rgba(0,0,0,0.08)', borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', color:'#9ca3af', textDecoration:'none', transition:'all 0.2s' }}
                ...
                <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                  {p.tags.map(t => <span key={t} className="kg-tag" style={{fontSize:11}}>{t}</span>)}
                </div>
              </div>
            </div>
          ))}
...
Object.assign(window, { KGProjects });
