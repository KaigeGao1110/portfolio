...
const contactItems = [
  ...
  { icon: <MapPinIcon/>, label: 'Chicago, IL', href: null },
];
  ...
  return (
    <section id="contact" style={{ padding:'var(--section-py) var(--section-px)', background:'#faf8f5' }}>
      <div style={{ width:'100%', maxWidth:'min(100%,1400px)', margin:'0 auto', textAlign:'center' }}>
        ...
        {/* Contact card */}
        <div style={{ background:'rgba(255,255,255,0.85)', backdropFilter:'blur(12px)', WebkitBackdropFilter:'blur(12px)', border:'1px solid rgba(0,0,0,0.08)', borderRadius:16, padding:'clamp(1.25rem,3vw,2.5rem)', marginBottom:32, boxShadow:'0 1px 3px rgba(0,0,0,0.06),0 4px 16px rgba(0,0,0,0.04)' }}>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:24 }}>
            {contactItems.map(item => {
              const inner = (
                <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:12, padding:'16px', borderRadius:12, transition:'background 0.2s', cursor: item.href ? 'pointer' : 'default' }}>
              ...
              return item.href ? (
                <a key={item.label} href={item.href} target={item.href.startsWith('http') ? '_blank' : undefined} rel="noopener noreferrer"
                   style={{ textDecoration:'none', borderRadius:12, display:'block', transition:'background 0.2s' }}
        ...
        <footer style={{ marginTop:96, paddingTop:32, borderTop:'1px solid #e5e7eb' }}>
          <p style={{ fontSize:12, color:'#9ca3af' }}>© 2026 Kaige Gao. Built with Next.js & framer-motion.</p>
        </footer>
      </div>
    </section>
  );
...
Object.assign(window, { KGContact });
